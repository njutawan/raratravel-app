// ===========================================================================
// auth-user-sync.ts — berkas SIAP TEMPEL untuk Supabase Dashboard (tanpa CLI).
//
// Dihasilkan otomatis dari supabase/functions/auth-user-sync/index.ts + _shared/*.ts
// oleh tools/bundle_functions.js. JANGAN diedit manual — ubah berkas asli lalu
// buat ulang:  node tools/bundle_functions.js auth-user-sync
//
...
// supabase/functions/auth-user-sync/index.ts
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "unregister-device") {
      ...
      return json({ ok: true, ...result2 }, { origin });
    }
    const result = await rpc("auth_user_sync", {
...
async function userId(firebaseUid) {
  const existing = await rpc("resolve_user_id", { p_firebase_uid: firebaseUid });
  if (existing) return existing;
  const created = await rpc("auth_user_sync", { p_firebase_uid: firebaseUid });
