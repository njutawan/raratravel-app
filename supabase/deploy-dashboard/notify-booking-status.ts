// ===========================================================================
// notify-booking-status.ts — berkas SIAP TEMPEL untuk Supabase Dashboard (tanpa CLI).
//
// Dihasilkan otomatis dari supabase/functions/notify-booking-status/index.ts + _shared/*.ts
// oleh tools/bundle_functions.js. JANGAN diedit manual — ubah berkas asli lalu
// buat ulang:  node tools/bundle_functions.js notify-booking-status
//
...
async function accessToken() {
  ...
  const now = Date.now();
  if (tokenCache && tokenCache.key === account.client_email && tokenCache.expiresAt > now + 6e4) {
  ...
  const claims = base64Url(JSON.stringify({
    ...
    exp: Math.floor(now / 1e3) + 3600
  }));
  const key = await crypto.subtle.importKey(
    "pkcs8",
    pemToBytes(account.private_key).buffer,
...
Deno.serve(async (req) => {
  ...
  try {
    if (!await isAuthorized(req)) {
      return errorResponse("forbidden", "Pemanggil tidak berwenang", 403, void 0, origin);
    }
    const body = await readJson(req);
    const jobs = [];
    if (body.job_id) {
      const job = await rpc("prepare_notification_job", { p_job_id: body.job_id });
      if (job) jobs.push(job);
    } else {
      const claimed = await rpc("claim_notification_jobs", {
...
async function isAuthorized(req) {
  ...
  try {
    ...
    return await rpc("staff_role", { p_user_id: userId }) !== null;
  } catch {
    return false;
  }
}
