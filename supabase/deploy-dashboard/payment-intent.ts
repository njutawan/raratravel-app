// ===========================================================================
// payment-intent.ts — berkas SIAP TEMPEL untuk Supabase Dashboard (tanpa CLI).
//
// Dihasilkan otomatis dari supabase/functions/payment-intent/index.ts + _shared/*.ts
// oleh tools/bundle_functions.js. JANGAN diedit manual — ubah berkas asli lalu
// buat ulang:  node tools/bundle_functions.js payment-intent
//
...
async function createMidtransSnapTransaction(params) {
  ...
  const base = env().midtransIsProduction ? "https://app.midtrans.com/snap/v1/transactions" : "https://app.sandbox.midtrans.com/snap/v1/transactions";
  const response = await fetch(base, {
    ...
    body: JSON.stringify({
      ...
      customer_details: {
        ...
        phone: params.customerPhone ?? void 0
      },
      credit_card: { secure: true },
      expiry: params.expiresAt ? { start_time: void 0, unit: "minutes", duration: 60 } : void 0
    })
  });
  if (!response.ok) {
    throw Object.assign(
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "manual-confirm") {
      ...
      let created2;
      try {
        created2 = await rpc("admin_create_payment", {
      ...
      const eventId = `manual:${created2.payment.id}:${created2.payment.amount}`;
      const applied = await rpc("apply_payment_event", {
        p_provider: "manual",
        p_event_id: eventId,
        p_event_type: "manual.transfer_received",
        p_status: "paid",
        p_provider_reference: created2.payment.provider_reference,
        p_amount: created2.payment.amount,
        p_payload: { confirmed_by: role, confirmed_at: (/* @__PURE__ */ new Date()).toISOString() },
        p_signature_valid: true,
        // staf terverifikasi = pengganti tanda tangan provider
        p_failure_reason: null
      });
      return json({ ok: true, confirmed_by: role, ...applied }, { origin });
    }
    const provider = body.provider ?? "manual";
    let created = await rpc("create_payment", {
      p_user_id: userId,
      p_kode: body.kode ?? null,
      p_provider: provider,
    ...
    if (provider === "midtrans") {
      ...
      created = await rpc("create_payment", {
        p_user_id: userId,
        p_kode: body.kode ?? null,
        p_provider: provider,
        p_method: body.method ?? null,
...
function instructionsFor(provider, created) {
