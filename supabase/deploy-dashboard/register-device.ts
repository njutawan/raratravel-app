// ===========================================================================
// register-device.ts — berkas SIAP TEMPEL untuk Supabase Dashboard (tanpa CLI).
//
// Dihasilkan otomatis dari supabase/functions/register-device/index.ts + _shared/*.ts
// oleh tools/bundle_functions.js. JANGAN diedit manual — ubah berkas asli lalu
// buat ulang:  node tools/bundle_functions.js register-device
//
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let userId = await rpc("resolve_user_id", {
    ...
    if (action === "unregister") {
      const result = await rpc("unregister_user_device", {
        p_user_id: userId,
